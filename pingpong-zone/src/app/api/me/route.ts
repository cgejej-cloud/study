...
export async function GET() {
  ...
  const user = await prisma.user.findUnique({
    where: { id: session.id },
    select: { id: true, name: true, email: true, role: true, nickname: true, bio: true, profileColor: true, phone: true, emailNotify: true, avatar: true, racketType: true, playStyle: true, title: true, agreedMarketing: true, agreedMarketingAt: true, notifyMatch: true, notifyChallenge: true },
  });
...
export async function PATCH(req: NextRequest) {
  ...
  const { name, nickname, bio, profileColor, phone, currentPassword, newPassword, emailNotify, avatar, racketType, playStyle, title, agreedMarketing, notifyMatch, notifyChallenge } = await req.json();
  ...
  if (typeof notifyMatch === "boolean") updateData.notifyMatch = notifyMatch;
  if (typeof notifyChallenge === "boolean") updateData.notifyChallenge = notifyChallenge;
